...
class Solution:
    def maxTree(self, A):
        ...
        for ele in A:
            node = TreeNode(ele)
            # 因为维护的是一个递减栈，所以如果当前元素大于等于栈顶，那么我就pop
            while stack and ele >= stack[-1].val:
                # 当我们pop元素时，意味着当前栈顶元素时左边第一个比当前元素小的，那么栈顶元素一定是当前元素的左子树
                node.left = stack.pop()
            # 执行完while后，栈顶元素一定比当前元素大，所以当前元素一定是栈顶元素的右子
            if stack:
                stack[-1].right = node
            stack.append(node)

        # 根节点时最大的，没有任何元素能够把根节点pop出去，所以栈顶就根节点
        return stack[0]
'''
...
以上得出结论：每个节点的父亲节点，是其左边和右边第一个比它大的数的中的比较小的那个．显然数据结构是使用单调递减栈
'''