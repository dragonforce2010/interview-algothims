    ...
    def expressionExpand(self, s):
        ...
        for char in s:
            ...
            if char.isdigit():
                ...
                # we push the number into the stack right after we found [, so that we can know the boundy when we need to pop out the elements when face ]. when we read number, we know the we have pop out the all the elements in []
                # if we don't put number into the stack, it's hard to find out the left boundy
                stack.append(number)
                number = 0
            elif char == ']':
                strs = []
                while stack:
                    elem = stack.pop()
                    if type(elem) == int:
                        # need to reverse since the stack is FILO
                        stack.append(''.join(reversed(strs)) * elem)
                        break
                    strs.append(elem)
            else:
                stack.append(char)
        # we use stack not only for it's FILO, but also use it to save the result
        return ''.join(stack)
